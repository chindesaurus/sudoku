...
struct {
    ...
    // remember the given numbers on the board
    bool given[DIM][DIM];
...
/*
 * Allow the user to replace any blank on the board with a number.
 */
void
enter_number(int i) {
    g.board[g.y][g.x] = i - '0';
...
load_board() {
    ...
    if (fread(g.board, 81 * sizeof(int), 1, fp) != 1) {
     ...
     // if given square is empty, store false in array given, else true
    for (int i = 0; i < DIM; i++) {
        for (int j = 0; j < DIM; j++) {
            if (g.board[j][i] == 0)
                g.given[j][i] = false;
